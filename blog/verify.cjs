...
for(const p of ctx.window.MIRA_DATA.products)assert.deepEqual(data[p.id],{name:p.name,imageUrl:p.imageUrl,available:!!p.affiliateUrl});
...
for(const name of fs.readdirSync(path.join(root,'_artigos'))){
 ...
 for(const id of m.produtos)assert.ok(data[id],`Produto inexistente: ${id}`);
 featured+=m.destaque?1:0;count++;
}
